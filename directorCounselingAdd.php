<?php include "server.php";
    ...
    $referralid = mysqli_real_escape_string($link,$_POST['referral-id']);
    $studentid = mysqli_real_escape_string($link,$_POST['student-id']);
    session_start();
    $setterid = $_SESSION['userid'];
    $sessionAddQuery = "INSERT INTO session(sessiondesc,action,counselorid,date,time,endtime,dateset,setter,status,referralid,studentid) VALUES('$desc','$action','$counselorid','$date','$timeStart','$timeEnd',NOW(),'$setterid','Upcoming','$referralid','$studentid')";
    mysqli_query($link,$sessionAddQuery);

    $referralUpdateQuery = "UPDATE referral SET status='Scheduled' WHERE id='$referralid'";
    mysqli_query($link,$referralUpdateQuery);
